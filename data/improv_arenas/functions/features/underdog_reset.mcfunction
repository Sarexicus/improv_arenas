...
execute as @a[tag=underdog] run scoreboard players operation @s iarena_resetv = reset_value iarena_var
execute as @a[tag=underdog] run scoreboard players operation @s iarena_resetv -= @s iarena_suicides
...
execute as @a[tag=underdog] run tellraw @s ["",{"text":"[improv_arenas] ","color":"green"},{"text":"underdog: "},{"text":"[⏪ Reset]","color":"green","clickEvent":{"action":"run_command","value":"/trigger iarena_resetp"}},{"text":" (for "},{"score":{"name":"round","objective":"iarena_var"}},{"text":" items, "},{"score":{"name":"@s","objective":"iarena_resetv"}},{"text":" point(s))"}]
