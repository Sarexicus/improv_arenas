...
execute as @a[tag=underdog] run scoreboard players operation @s points += underdog_pointvalue cvar

#tellraw to announce underdog
execute if entity @a[tag=underdog] run tellraw @a ["",{"text":"[improv_arenas] ","color":"green"},{"text":"underdog: "},{"score":{"name":"underdog_pointvalue","objective":"cvar"}},{"text":" extra point(s) for "},{"selector":"@a[tag=underdog]"}]
#cleanup
